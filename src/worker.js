...
async function handleAPI(request, env) {
  ...
  try {
    ...
    const fullUrl = env.URL
      ? `${env.URL}${filePath}`
      : `${new URL(request.url).origin}${filePath}`;
    ...
    if (password === env.PASS) {
      return new Response('', {
        ...
        headers: {
          Location: '/',
          'Set-Cookie':
            'auth=true; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=86400',
        },
      });
    } else {
      return new Response(
        PASSWORD_TEMPLATE.replace(
          '</form>',
          '<div class="error">密码错误</div></form>'
        ),
        {
          headers: { 'Content-Type': 'text/html; charset=utf-8' },
        }
